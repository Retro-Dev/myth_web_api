...
class Event extends RestModel {
    ...
    static getFields()
    {
        return ['title', 'status','description','address', 'date', 'time','slug','file_url', 'type'];
    }
    ...
    static async getEvent(condition) {
      let event = await this.query().where(condition).first()
      event = event ? event.toJSON() : []
      return event
  }

    static async updateEvent(condition, data) {
