...
const DelOtherPanic = use("App/Models/DelOtherPanic");
const Antl = use("Antl");
const _ = use("lodash");

class WitnessController extends RestController {
  constructor() {
    super("Witness"); //this is your model name
    this.resource = "Witness"; //this is your resource name
  ...
  async beforeStoreLoadModel() {
    ...
    // delete record in other_panic_deleted_by
    await DelOtherPanic.query()
      .where("panic_id", params.panic_id)
    ...
    // get witness record
    let witness = await Witness.getWitness(condition);


    // check if witness empty
    if (!_.isEmpty(witness)) {
      this.__is_error = true;
      this.__is_paginate = false;
      return this.sendResponse(
        200,
        Antl.formatMessage("messages.success_store_message"),
        witness
      );
    }
  }
