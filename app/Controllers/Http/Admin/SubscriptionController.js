...
class SubscriptionController extends Controller {
    async index({ view }) {
        return view.render('admin.subscription.index')
    }
    ...
    async ajaxListing({ request, response }) {
        ...
        let dataTableRecord = await Subscription.dataTableRecords(request);
        ...
        if (dataTableRecord['records'].length > 0) {
            for (var i = 0; i < dataTableRecord['records'].length; i++) {
                record_data.push([
                    ...
                    dataTableRecord['records'][i].user.name,
                    dataTableRecord['records'][i].title,
                    dataTableRecord['records'][i].purchase_date,
                    dataTableRecord['records'][i].expiry_date,
                    `<a href="${baseUrl('/admin/subscription/edit/' + dataTableRecord['records'][i].slug)}" class="btn btn-sm btn-info"><i class="fa fa-eye"></i></a>`
                ])
            }
    ...
    async edit({ request, response, session, view, params }) {
        ...
        let record = await Subscription.getRecordBySlug(request, params.slug);
        ...
        if (_.isEmpty(record)) {
            ...
            response.route('admin.subscription');
            return;
        }
        
        return view.render('admin.subscription.edit', { record: record });
    }
